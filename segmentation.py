...
from model import MiniUNet, UNet
...
def save_learning_curve(train_loss_list, train_miou_list, val_loss_list, val_miou_list):
    ...
    plt.ylabel('mIoU')
    plt.grid(True)
    plt.savefig('extra_learning_curve.png', bbox_inches='tight')
    plt.show()
...
def train(model, device, train_loader, criterion, optimizer):
    ...
    for batch_id, sample_batched in enumerate(train_loader):
        image = sample_batched['input'].to(device)
        mask = sample_batched['target'].to(device)

        outputs = model(image)
        train_iou += sum(iou(outputs, mask))/len(iou(outputs, mask))
...
def val(model, device, val_loader, criterion):
    ...
    with torch.no_grad():
        for batch_id, sample_batched in enumerate(val_loader):
            ...
            outputs = model(image)
            val_iou += sum(iou(outputs, mask))/len(iou(outputs, mask))
...
def main():
    ...
    epoch, max_epochs = 1, 25  # TODO: you may want to make changes here
    best_miou = float('-inf')
    #m, e, starting_miou = load_chkpt(model, 'checkpoint.pth.tar')
